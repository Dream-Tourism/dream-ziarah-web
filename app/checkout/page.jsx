...
const CheckoutPage = () => {
  ...
  const { data, isSuccess: logoSuccess, isLoading: logoLoading } = useGetLogoUrlQuery(null)

  const logoUrl = logoSuccess ? data?.general_settings?.[0]?.cloudflare_favicon : ""
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        document.cookie = "booking_info=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;"
        document.cookie = "channel_id=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;"
      } else {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<i key={i} className="icon-star text-yellow-1 text-10" style={{ fontSize: "12px" }}></i>)
    }

    if (hasHalfStar) {
      stars.push(<i key="half" className="icon-star text-yellow-1 text-10" style={{ fontSize: "12px" }}></i>)
    }

    const remainingStars = 5 - Math.ceil(rating)
    for (let i = 0; i < remainingStars; i++) {
      stars.push(<i key={`empty-${i}`} className="icon-star text-yellow-1 text-10" style={{ fontSize: "12px" }}></i>)
    }
  ...
  const getInputStyling = (fieldName) => {
    ...
    return {
      padding: "12px 16px",
      border: hasError ? "2px solid #dc3545" : isValid ? "2px solid #28a745" : "1px solid #007bff",
      boxShadow: hasError
      ...
      >
        <div className="container d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex">
            ...
            >
              <Image style={{ cursor: "pointer" }} src="/img/whatsapp.svg" width={50} height={50} alt="WhatsApp" />
            </a>
          </div>
        </div>
      </div>
                  ...
                  <div className="row g-3 mb-4">
                    ...
                    <div className="col-12">
                      <label className="form-label">
                        ...
                        <small className="text-muted ms-2">We'll send your tickets here</small>
                      </label>
                      <input
                        type="email"
                        className="form-control"
                ...
                <div className="bg-white rounded shadow-sm p-4">
                  ...
                  <div className="card border-0 mb-3">
                    <div className="row g-0">
                      <div className="col-4">
                        <img
                          src={bookingData.tourImage || "/placeholder.svg?height=80&width=120" || "/placeholder.svg"}
                      ...
                      <div className="col-8">
                        <div className="card-body p-3">
                          <h6 className="card-title mb-1" style={{ fontSize: "14px", lineHeight: "1.3" }}>
                            {bookingData.tourName}
                          </h6>
                          <div className="d-flex align-items-center mb-2">
                            <span className="me-1 fw-bold" style={{ fontSize: "12px" }}>
                              {bookingData.rating}
                            </span>
                            <div className="me-2">{renderStars(bookingData.rating)}</div>
                            <small className="text-muted">({bookingData.reviewCount})</small>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mb-3">
                    <div className="d-flex align-items-center mb-2">
                      <i className="fas fa-calendar-alt text-muted me-2" style={{ fontSize: "14px" }}></i>
                      <span style={{ fontSize: "14px" }}>
                        {formatDate(bookingData.selectedDate)} at {getTimeRange()}
                      </span>
                    </div>
                    <div className="d-flex align-items-center mb-2">
                      <i className="fas fa-users text-muted me-2" style={{ fontSize: "14px" }}></i>
                      <span style={{ fontSize: "14px" }}>
                        {bookingData.participantCount} Participants ({bookingData.selectedTourType?.guide})
                      </span>
                    </div>
                    <div className="d-flex align-items-center mb-2">
                      <i className="fas fa-user-tie text-muted me-2" style={{ fontSize: "14px" }}></i>
                      <span style={{ fontSize: "14px" }}>Tour Type: {bookingData.selectedTourType?.guide}</span>
                    </div>
                    <div className="d-flex align-items-center text-success">
                      <i className="fas fa-check-circle me-2" style={{ fontSize: "14px" }}></i>
                  ...
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <span></span>
                    <h5 className="mb-0">US${bookingData.totalPrice.toFixed(2)}</h5>
                  </div>

                  <div className="mb-3">
                    <a href="#" className="text-primary text-decoration-none" style={{ fontSize: "14px" }}>
                  ...
                  <div className="border-top pt-3">
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <h4 className="mb-0">US${bookingData.totalPrice.toFixed(2)}</h4>
                    </div>
                    <small className="text-success">All taxes and fees included</small>
                  </div>
                </div>
              </div>
            </div>
